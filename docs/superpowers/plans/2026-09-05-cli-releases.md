...
- Follow-up release workflow correction: `release-build` creates a missing annotated local tag at the clean, committed `HEAD` after source/tool/environment validation. An existing tag selects its own commit unchanged, including when `HEAD` differs. Print the chosen tag and full commit; never fetch, push, move, or convert tags during a local build. Preserve a newly created tag if packaging later fails.
- All commands that build or change release/operator state require a clean invoking checkout, including staged, unstaged, and untracked files. Apply one early guard to release `tools`, `snapshot`, `build`, `publish` (also with `--dist`), `resume`, native-record commands, and acceptance-template creation, plus `make build`, `make tidy`, and `make get`. Checks, verification, status, tests, vet, and interactive run/dev remain usable while editing. Portable native smoke collectors do not require a Git checkout. Recovery from retained records remains valid after branch advancement when the invoking checkout is clean.
...
Keep `release/environments.json` tracked and reviewed because endpoints are public build metadata. The approved values are now committed:

```json
{
  "schema_version": 1,
  "repository": "bgr11n/hookspot-cli",
  "stage": { "server_url": "https://app.stage-m0rk7a.hookspot.dev/", "branch": "stage" },
  "prod": { "server_url": "https://app.hookspot.dev/", "branch": "main" }
}
```

Empty URLs still fail release validation. Committed endpoints identify the intended services; their presence does not prove backend acceptance or publication eligibility. The repository value is based on the observed origin and must be confirmed before publishing. Do not substitute example domains as real values. Select the record with an explicit `ENV=stage|prod`; export its canonical URL as `SERVER_URL` only to the build container. Reject an externally supplied generic `SERVER_URL` if it differs from that record. If it matches, accept it only as a redundant assertion, not as an override.
...
**Files:** create `release/toolchain.env`, `Dockerfile.release`; modify `go.mod`, `go.sum` only as required by the toolchain/dependency checks, `Makefile`, `Dockerfile`, `docker-compose.yml`, `.air.toml`, `.gitignore`, `.dockerignore`, `README.md`. No file removal. **Dependencies:** none.
...
- [ ] Build the local tool image, for example `hookspot-release:go1.26.8-gr2.17.1-gh2.100.0`, using Docker only. Expose `make release-tools` as a thin script call once Task 7 is added. Until then, use an explicit `docker build -f Dockerfile.release ...` invocation with values from the tracked version file.
...
The fixture domain supplies test data only; real releases use the approved endpoints in the committed manifest. Add the invalid-input and redirect cases enumerated above to exercise failures, rather than asserting implementation string operations.
...
- [ ] Validate parameters, Git repository/full history, manifest, tool compatibility, and clean tree before building. Reject shallow, partial/promisor, replacement-ref and legacy-graft source views before reading selected blobs, with guidance to prepare a normal full local checkout. A nonshallow partial clone can still auto-fetch during git show; a replacement ref can make preflight and clone see different contents for one SHA. Use the original-object view consistently. One early cleanliness guard must reject staged, unstaged, and untracked changes for `tools`, `snapshot`, `build`, `publish` (including `--dist`), `resume`, `native-requirements`, `native-review-template`, `native-manual`, `native-evidence`, and `acceptance-template`, plus `make build`, `make tidy`, and `make get`, before builds, tags, locks, publication, or operator-record writes. Users can review and commit on a temporary branch. Do not offer a dirty-checkout override. `check`, `verify`, `status`, tests, vet, and run/dev remain usable while dirty; publication-policy compatibility is a separate check. Portable smoke collectors must not acquire a Git-checkout prerequisite.
...
- [ ] Tagged build: validate the explicit tag/channel. If the local tag is missing, resolve clean committed `HEAD` once, validate that source's release interface, toolchain, and environment, then create an annotated local tag at the pinned commit. If it exists, peel and build its exact commit even when `HEAD` differs; preserve its existing tag object and annotation style. Print the selected tag and full commit. Set `GORELEASER_CURRENT_TAG` and run `goreleaser release --config .goreleaser.yaml --skip=publish` without skipping tag validation. Nonpublishing build never fetches, pushes, moves, or converts tags and needs no GitHub token. A tag created before a later packaging failure remains; changed source requires a new candidate tag.
...
**Tests:** isolated Git fixture repositories and fake Docker/GitHub executables record operations. Assert snapshot/check do not create/delete/change tags; tagged build creates only a missing annotated tag at validated `HEAD` and preserves an existing tag even when its commit differs from `HEAD`. None fetch, push, or publish, even with token variables in the parent environment. Read-only tag inspection is allowed: pinned GoReleaser itself uses `git tag -l` and `git tag --points-at`. Missing env, invalid tag, and incompatible source/tool/environment inputs must fail before tag creation; subsequent build failure preserves the new tag. Test staged, unstaged, and untracked changes against every guarded entry point and confirm no build, tag, lock, or operator/publication write occurs. Checks and retained verification remain available while dirty. Duplicate output paths and failed tests must fail without remote writes. Real integration builds remain in Docker; the fake harness validates operation boundaries rather than YAML text.

**Compatibility:** old `make stage-release SERVER_URL=...` without explicit TAG/approved manifest now fails with migration guidance. Tagged build now creates a missing annotated tag at validated committed `HEAD`; existing tags keep their identity. All guarded operations require committed changes, including retained-output publication/resume. New local source/output copies are temporary; retained artifact parents survive failure for diagnosis and resume.
...
- [ ] Obtain a local per-repository publication lock (`mkdir` with owner metadata, trap cleanup; no automatic stale-lock theft) after the common cleanliness guard. Before creating any remote state, run feasible checks/tests, create or verify the local annotated tag at the pinned SHA, build the tagged bundle token-free, verify it, collect required native evidence, and prepare reviewed release notes. Reuse an existing remote tag object exactly; equality of peeled commits does not authorize a different annotated tag object. Both tagged build and publish may create a missing local annotated tag after their preflight; snapshot never creates one. No tag is ever moved. Persist initial publication identity and completed eligibility checks atomically before tag push so a lost push response followed by branch advancement remains recoverable from a clean invoking checkout.
...
| `make release-build ENV=stage TAG=v0.1.0-stage.1` | Build/package the tag's exact commit; create a missing annotated tag at validated committed `HEAD` | Creates a missing local tag only; no push/publication | No |
...
`REF` is shell notation for an existing full commit/ref, not an arbitrary string passed to eval. `TAG` is always explicit for tagged operations; `GORELEASER_CURRENT_TAG` prevents ambiguity when stage and prod tags point to the same commit. New tags created by tagged build or publish are annotated. An existing tag is never converted or moved; local tagged builds select its own commit even when `HEAD` differs. The chosen tag/full commit is printed, and a newly created tag survives later build failure.

Tool builds, snapshot/tagged builds, publish (including with `DIST`), resume, native-record commands, and acceptance-template creation reject any staged, unstaged, or untracked changes before work. `make build`, `make tidy`, and `make get` use the same rule. Checks, verification, status, tests, vet, and run/dev remain usable while dirty. The native smoke collectors do not require a Git checkout. A clean checkout may resume a recorded release after the branch has advanced; recovery remains bound to its retained commit and bytes.
...
# Choose the intended version; a missing tag is created at validated HEAD.
...
| Tagged packages | `make release-build ENV=stage TAG=v0.1.0-stage.1` and prod equivalent | Missing tag is annotated at validated committed `HEAD`; existing tag object/commit is preserved even when `HEAD` differs. Exact chosen tag/SHA, expected version/kind, all six targets; no fetch/push/publication. Same-SHA multiple tags do not confuse selection. |
...
| Source/tag failures | `scripts/release_test.sh` isolated Git fixtures | Invalid tag/channel or incompatible source/tool/environment fails before local tag creation. Wrong publication branch/source, missing stage evidence, changed prod SHA, and moved remote tag fail without unauthorized writes. |
| Clean-checkout guard | Direct-script and Make fixture commands with staged, unstaged, and untracked changes | Every guarded build/mutation fails before work; checks/verify/status/test/vet/run/dev remain available. Clean retained recovery works after branch advancement. |
...
4. All source/tag/environment/URL/config mismatch cases fail before network authentication or remote publication as applicable. Snapshot/check never create tags. Tagged build creates only a missing annotated local tag after validation and preserves existing tags. These local commands never fetch/push tags or publish, even with ambient tokens. Every guarded build/mutation rejects staged, unstaged, and untracked changes before work; checks and verification remain usable while dirty.
